  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {!loading && workouts.length > 0 && (() => {
        ...
        const cycleInfo = user?.gender === "FEMALE" && user?.menstrualTrackingEnabled && user?.lastPeriodStart 
          ? getCycleInfo(user.lastPeriodStart, user.cycleLength || 28) 
          : null;
        
        return (
          <div className="mt-4 space-y-3">
            <h2 className="text-lg font-semibold">Recent Logs</h2>
            {cycleInfo && (cycleInfo.phaseKey === "menstrual" || cycleInfo.phaseKey === "luteal") && (
              <div className="rounded-2xl border border-rose-500/20 bg-rose-500/10 p-4 text-xs font-semibold leading-relaxed flex items-center gap-3 text-rose-500 dark:text-rose-400 animate-in fade-in slide-in-from-top-2 duration-300">
                <span className="text-lg">🌸</span>
                <p>
                  You are currently in your <strong>{cycleInfo.phaseName}</strong>. Strength fluctuations and shifts in fatigue are a natural biological response. Honor your body and adjust training loads accordingly.
                </p>
              </div>
            )}
